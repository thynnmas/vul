...
typedef struct csp_graph_node_t {
   csp_graph_pos_t pos;
   vul_svector *neighbors; // index into csp_graph_t->nodes. 
} csp_graph_node_t;
...
csp_graph_t *graph_input_from_string( u32 *vert_count, u32 *edge_count, u32 K, char *str )
{
   ...
   for( a = 0; a < *vert_count; ++a ) {
      ...
      node->pos.x = x;
      node->pos.y = y;
      node->neighbors = vul_svector_create( sizeof( u32 ), K,
                                  malloc, free ); // If solvable, we're bounded by K
   }
...
void csp_graph_create_astar( vul_astar_graph *agraph, csp_graph_t *graph, u32 K )
{
   ...
   for( i = 0; i < vul_svector_size( graph->nodes ); ++i ) {
      node = ( csp_graph_node_t* )vul_svector_get( graph->nodes, i );
      for( j = 0; j < vul_svector_size( node->neighbors ); ++j ) {
         idnode2 = *( u32* )vul_svector_get( node->neighbors, j );
...
void csp_graph_finalize_astar( vul_astar_graph *graph )
{
   ...
   for( i = 0; i < vul_svector_size( cg->nodes ); ++i ) {
      ...
      vul_svector_destroy( n->neighbors );
   }
...
{
   switch( strategy ) {
   case VUL_ASTAR_STRATEGY_BEST_FIRST:
      printf("Best first\n");
      break;
   case VUL_ASTAR_STRATEGY_DEPTH_FIRST:
      printf("Depth first\n");
      break;
   case VUL_ASTAR_STRATEGY_BREADTH_FIRST:
