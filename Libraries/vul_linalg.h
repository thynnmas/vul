 ...
 * this decomposition (or use the @TODO(thynn): LU(0) or LUT
 * preconditioners).
 *
 ...
 * Iterative solver of the linear system Ax = b given a LL* decomposition
 * of the matrix A. Use vul_linalg_cholesky_decomposition_sparse to create
 * this decomposition (or use the @TODO(thynn): incomplete cholesky decomposition
 * preconditioner).
 *
 * This method employs at most max_iterations of iterative refinement, or until
 * the average square error is below tolerance.
	...
	{\
		...
		while( ia < a->count && ib < b->count ) {\
			if( a->entries[ ia ].idx == b->entries[ ib ].idx ) {\
				...
				vul_linalg_vector_insert( out, a->entries[ ia ].idx, 0.f op a->entries[ ia ].val );\
				++ia;\
			} else {\
				vul_linalg_vector_insert( out, b->entries[ ib ].idx, 0.f op b->entries[ ib ].val );\
				++ib;\
			}\
		}\
		while( ia < a->count ) {\
	...
	for( i = 0; i < max_iterations; ++i ) {
		...
		for( j = 0, k = 0; j < p->count && k < r->count; ) {
			if( p->entries[ j ].idx == r->entries[ k ].idx ) {
				...
				vul_linalg_vector_insert( p, r->entries[ k ].idx, r->entries[ k ].val );
				++k;
			}
		}
		rd = rd2;
...
// @TODO(thynn): Preconditioners, see http://www2.imm.dtu.dk/~apek/ITSOL2011/material/tuesday/Lecture_2.pdf

// @TODO(thynn): Support these in CG as well (= call it pcg), and test them independently of the solvers.
// @TODO(thynn): Preconditioners + GMRES for dense matrices!
typedef enum vul_linalg_precoditioner_type {
	...
	VUL_LINALG_PRECONDITIONER_Count,
...
/*
 * If left is set, we apply the preconditioner on the left, otherwise we
 * apply it on the right. @TODO(thynn): Is this actually sensible? Need bigger test
 * problems to evaluate on!
 * For now this has the following effect based on which preconditioner is applied:
 * jacobi: Copy on left, apply on right
 * cholesky: Apply L^T on left, apply L on right
 * ILU(0): Apply U on left, apply L on right
 * These are chosen so they give correct results (except for ILU(0) which is bugged),
 * and converge fastest of possible apply-partial permutations!
 * @TODO(thynn): Figure out why LU is wrong, even when we use the full LU decomp!
 */
void vul__linalg_precondition_solve( vul_linalg_precoditioner_type type,
												 vul_linalg_vector *x,
												 vul_linalg_matrix *P, vul_linalg_vector *b, int left )
{
	int i;

	vulb__sparse_vclear( x ); // @TODO(thynn): Is this redundant?
	...
	if( left ) {
		...
		case VUL_LINALG_PRECONDITIONER_INCOMPLETE_CHOLESKY: {
			...
			/* Backward substitute */
			// @TODO(thynn): This retrieves zeroes, optimize it so it doesn't!
			for( i = P->count - 1; i >= 0; --i ) {
				vul_linalg_real sum = vul_linalg_vector_get( b, i );
				for( j = i + 1; j <= m; ++j ) {
...
//@TODO(thynn): Comment that this is _left preconditioning_!
														  ...
														  vul_linalg_real tolerance )
{
	...
	// @TODO(thynn): Preconditioners lead to slower convergence; find out if wrong, or
	// just a bad testmatrix for this!
	// @TODO(thynn): See http://www.pvamu.edu/include/Math/AAM/AAM%20Vol%208,%20Issue%201%20(June%202013)/15_Rahmani%20AAM-R534-MR-082512%20Ready%20to%20Post%2006-25-13.pdf
	// for potentially faster/better version
	...
	vul__linalg_precondition_solve( ptype, r, P, w, 1 );
	...
	for( k = 0; k < max_iterations; ++k ) {
		...
		for( i = 0; i < restart_interval; ++i ) {
			// Replace with preconditioner solve!
			...
			if( l == -1 ) {
				ERR( "Row/column is zero, matrix singular? TODO: Figure out what to do here, if this is error!" );
				return x;
			}
			vulb__sparse_vclear( w );
			vulb__sparse_vclear( y );
			vul__linalg_precondition_solve( ptype, y, P, &V->rows[ l ].vec, 0 );
			...
			for( j = 0; j <= i; ++j ) {
				...
				// @TODO(thynn): Optimize this, only iterate over non-zero entries
				if( w->count > 0 ) {
#define MAX(a,b) ((a)>=(b)?(a):(b))
					m = MAX( w->entries[ w->count ].idx, V->rows[ j ].vec.entries[ V->rows[ j ].vec.count ].idx );
#undef MAX
					for( l = 0; l < m; ++l ) {
						vul_linalg_vector_insert( w, l, vul_linalg_vector_get( w, l )
																- tmp * vul_linalg_matrix_get( V, j, l ) );
					}
				}
			}
			...
			if( err <= tolerance ) {
				...
				// @TODO(thynn): Optimize this (only iterative over the ones that are non-zero)
				for( j = 0; j < i; ++j ) {
					vul_linalg_vector_insert( x, j, vul_linalg_vector_get( x, j )
															- vul_linalg_matrix_get( V, i, j )
															* vul_linalg_vector_get( y, j ) );
				}
				break;
			}
		}
		...
		vul__linalg_precondition_solve( ptype, y, P, r, 0 );
