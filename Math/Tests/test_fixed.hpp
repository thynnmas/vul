...
namespace vul_test {
	...
	bool TestFixed::ops( )
	{
		...
		assert( ( f32_t )abs( ( f32_t )fi ) == ( f32_t )abs( f ) );
		assert( ( f32_t )abs( ( f32_t )fim ) == ( f32_t )abs( ( f32_t )fi ) );
